...
class AVLTree(object):
    ...
    def _getMaxValue(self, root):
        if root is None:
            ...
            return root.key
        return self._getMaxValue(root.right)
    ...
    def popMaxValue(self):
        if self.size == 0:
            return None
        else:
            key = self._getMaxValue(self.root)
            self.delete(key)
            return key
...
for _ in range(int(input())):
    ...
    for i in tuple(map(int, input().split()))[:-1]:
        avl.insert(i)
    while avl.size > 1:
        min_value = avl.popMin()
        max_value = avl.popMaxValue()
        new_value = max_value - min_value
        if not avl.search(new_value):
