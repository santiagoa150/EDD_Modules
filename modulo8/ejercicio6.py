...
class AVLTree(object):
    ...
    def get_max_sub_tree(self):
        return self._get_max_sub_tree_recursively(self.root)

    def _get_max_sub_tree_recursively(self, root):
        if root is None:
        ...
        left_is_filled = self._get_max_sub_tree_recursively(root.left)
        right_is_filled = self._get_max_sub_tree_recursively(root.right)

        if left_is_filled[0] and right_is_filled[0]:
            if left_is_filled[1] == right_is_filled[1] and left_is_filled[2] and right_is_filled[2]:
                return True, root.height, True
            return True, max(left_is_filled[1], right_is_filled[1]), False

        return False, max(left_is_filled[1], right_is_filled[1]), False
